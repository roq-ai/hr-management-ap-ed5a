...
import { useRoqClient, useInterviewsFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<InterviewsInterface, unknown>;

interface InterviewsListPageProps {
...
export function InterviewsListPage(props: InterviewsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useInterviewsFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.interviews.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: InterviewsInterface) => {
    if (hasAccess('interviews', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'interviewer_id', header: 'interviewer_id', accessorKey: 'interviewer_id' },
    !hideActions
      ? {
          id: 'actions',
          header: '',
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('interviews', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="interviews" />
        </Flex>

        {hasAccess('interviews', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
